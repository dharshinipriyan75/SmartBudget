...
class BudgetRuleEngine {
  // ── Dynamic limits fetched from Hive ──────────────────────────────────────
  static Map<String, int> get categoryLimits =>
      CategoryService.getCategoryLimitsMap();  // ← dynamic now

  static int get totalBudget =>
      categoryLimits.values.fold(0, (a, b) => a + b);
  ...
  static List<RuleSuggestion> generate({
    ...
    final limits = categoryLimits; // snapshot for this run
    ...
    for (var entry in categorySpent.entries) {
      final double limit =
          (limits[entry.key] ?? 0) * (isYearly ? 12.0 : 1.0);
      final double spent = entry.value;
      final double pct = limit > 0 ? spent / limit : 0.0;

      if (pct >= 1.0) {
        final double overspend = spent - limit;
        final double reducePerDay =
            daysRemaining > 0 ? overspend / daysRemaining : overspend;
        results.add(RuleSuggestion(
    ...
    for (var entry in categorySpent.entries) {
      ...
      if (pct < 1.0 && pct > periodProgress + 0.10) {
        final double projectedEnd =
            periodProgress > 0 ? spent / periodProgress : spent;
    ...
    final double projectedSpend =
        periodProgress > 0 ? totalSpent / periodProgress : totalSpent;
...
class _SuggestionsPageState extends State<SuggestionsPage> {
  ...
  Widget build(BuildContext context) {
    // ── Listen to BOTH transactions AND categories boxes ──────────────────
    return ValueListenableBuilder(
      valueListenable: Hive.box<SBTransaction>('sb_transactions').listenable(),
      builder: (context, Box<SBTransaction> txnBox, _) {
        return ValueListenableBuilder(
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(
                            ...
                            children: [
                              const Text("🎯 Savings Target",
                                  style: TextStyle(
                                      color: Colors.white,
                                      fontSize: 15,
                                      fontWeight: FontWeight.bold)),
                              Text("${_savingsPct.toStringAsFixed(0)}%",
